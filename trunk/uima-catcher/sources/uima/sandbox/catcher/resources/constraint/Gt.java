...
public class Gt implements Constraint {
	...
	public Gt(Element left,Element right) {
		this.left = left;
		this.right = right;
	}
	...
	public int compareTo(Constraint constraint) {
		if (constraint instanceof Eq) {
			...
			Gt gt = (Gt) constraint;
			int diff = this.left.compareTo(gt.left());
			if (diff == 0) {
				return this.right.compareTo(gt.right());
			} else {
				return diff;
			}
		} else {
	...
	public boolean match(JCas cas, Map<String, Annotation> values) {
		...
		if (left instanceof Type && right instanceof Type) {
			...
			return cas.getTypeSystem().subsumes(second, first);
