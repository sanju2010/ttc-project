...
public class Leq implements Constraint {
	...
	public Leq(Element left,Element right) {
		this.left = left;
		this.right = right;
	}
	...
	public int compareTo(Constraint constraint) {
		if (constraint instanceof Eq) {
			...
			Leq leq = (Leq) constraint;
			int diff = this.left.compareTo(leq.left());
			if (diff == 0) {
				return this.right.compareTo(leq.right());
			} else {
				return diff;
			}
		} else {
	...
	public boolean match(JCas cas, Map<String, Annotation> values) {
		...
		if (left instanceof Type && right instanceof Type) {
			...
			return first.getName().equals(second.getName()) || cas.getTypeSystem().subsumes(first, second);
