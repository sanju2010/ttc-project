...
public class ForAll implements Constraint {
	...
	public ForAll(String variable, String type, Constraint constraint) {
	...
	public int compareTo(Constraint constraint) {
		if (constraint instanceof Exists) {
			...
			ForAll forAll = (ForAll) constraint;
			int diff = this.type.compareTo(forAll.type());
			if (diff == 0) {
				Map<String, Element> values = new HashMap<String, Element>();
				String name = this.variable + "+" + forAll.variable();
				values.put(name, new Variable(name));
				Constraint first = this.constraint.subst(values);
				Constraint second = forAll.constraint().subst(values);
	...
	public Constraint subst(Map<String, Element> values) {
		...
		return new ForAll(this.variable, this.type, constraint);
	}
	...
	public String toString() {
		return "forall " + this.variable + " :" + this.type + " . " + this.constraint.toString();
	}
