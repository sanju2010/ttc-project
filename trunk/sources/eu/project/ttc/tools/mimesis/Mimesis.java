...
public class Mimesis implements Runnable {
	...
	private MimesisSettings settings;
	
	private void setSettings() {
		this.settings = new MimesisSettings(System.getProperty("user.home") + File.separator + ".mimesis.settings");
	}
	
	public MimesisSettings getSettings() {
	...
	private void setPreferences() {
		this.preferences = new Preferences("mimesis.properties");
		try {
	...
	private void enableListeners() {
		MimesisEngineListener engineListener = new MimesisEngineListener();
		engineListener.setMimesis(this);
		...
		windowListener.setMimesis(this);
		this.getFrame().addWindowListener(windowListener);
	}
	
	private void process() {
		MimesisEngineListener listener = new MimesisEngineListener();
		listener.setMimesis(this);
		listener.doProcess();
	}
	...
	public static void main(String[] args) {
		...
		if (wrong == null) {
			Mimesis ziggurat = new Mimesis(cli);
	...
	private class WindowListener extends WindowAdapter {
		...
		public void windowClosing(WindowEvent event) {
			...
			String title = "Exit?";
			int response = JOptionPane.showConfirmDialog(this.getMimesis().getFrame(),message,title,JOptionPane.OK_CANCEL_OPTION);
			if (response == 0) {
