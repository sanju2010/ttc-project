...
public class FlxConverterEngine extends SwingWorker<CpeDescription,Void> implements ConverterEngine {
	...
	protected CpeDescription doInBackground() throws Exception {
		...
		File file = File.createTempFile("converter-flx-cpe-",".xml");
		file.deleteOnExit();
	...
	private void setAnalysisEngine() throws Exception {
		ConfigurationParameterSettings parameters = this.getConverterTool().getSettings().getMetaData().getConfigurationParameterSettings();
		URL url = this.getClass().getClassLoader().getResource("eu/project/ttc/all/engines/FlxCasConsumer.xml");
        CpeIntegratedCasProcessor termMateAnnotator = CpeDescriptorFactory.produceCasProcessor("FLX Converter Analysis Engine");
