...
public class TbxXmiConverterEngine extends SwingWorker<CpeDescription,Void> implements ConverterEngine {
	...
	protected CpeDescription doInBackground() throws Exception {
		...
		File file = File.createTempFile("converter-tbx-xmi-cpe-",".xml");
		file.deleteOnExit();
	...
	private void setCollectionReader() throws Exception {
		...
		URL url = this.getClass().getClassLoader().getResource("eu/project/ttc/all/engines/TbxCollectionReader.xml");
		...
		settings.setParameterValue("File",parameters.getParameterValue("InputFile"));
	...
	private void setAnalysisEngine() throws Exception {
        ...
        CpeIntegratedCasProcessor termMateAnnotator = CpeDescriptorFactory.produceCasProcessor("TBX to XMI Converter Analysis Engine");
