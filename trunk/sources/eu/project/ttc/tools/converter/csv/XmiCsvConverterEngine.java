...
public class XmiCsvConverterEngine extends SwingWorker<CpeDescription,Void> implements ConverterEngine {
	...
	protected CpeDescription doInBackground() throws Exception {
		...
		File file = File.createTempFile("converter-xmi-csv-cpe-",".xml");
		file.deleteOnExit();
	...
	private void setAnalysisEngine() throws Exception {
		ConfigurationParameterSettings parameters = this.getConverterTool().getSettings().getMetaData().getConfigurationParameterSettings();
		URL url = this.getClass().getClassLoader().getResource("eu/project/ttc/all/engines/CsvCasConsumer.xml");
        CpeIntegratedCasProcessor termMateAnnotator = CpeDescriptorFactory.produceCasProcessor("XMI to CSV Converter Analysis Engine");
        ...
        settings.setParameterValue("SortBy", (String) parameters.getParameterValue("SortBy"));
