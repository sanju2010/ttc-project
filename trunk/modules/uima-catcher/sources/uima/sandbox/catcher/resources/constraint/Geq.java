...
public class Geq implements Constraint {
	...
	public Geq(Element left,Element right) {
		this.left = left;
		this.right = right;
	}
	...
	public int compareTo(Constraint constraint) {
		if (constraint instanceof Eq) {
			...
			Geq geq = (Geq) constraint;
			int diff = this.left.compareTo(geq.left());
			if (diff == 0) {
				return this.right.compareTo(geq.right());
			} else {
				return diff;
			}
		} else {
	...
	public boolean match(JCas cas, Map<String, Annotation> values) {
		...
		if (left instanceof Type && right instanceof Type) {
			...
			return first.getName().equals(second.getName()) || cas.getTypeSystem().subsumes(second, first);
