...
import fr.univnantes.lina.uima.models.TermContextIndex;
...
public class TermContextComputer extends JCasAnnotator_ImplBase {
	...
	public void process(JCas cas) throws AnalysisEngineProcessException {
		try {
			...
			this.getContext().getLogger().log(Level.INFO,e.getMessage());
		} catch (Exception e) {
	...
	private void doIndex(JCas cas,String indexID,TermContextVectorAnnotation annotation) throws Exception {
		...
		String term = annotation.getTerm().getCoveredText();
		index.addOccurrences(term, null);
		for (int i = 0; i < length; i++) {
			String context = annotation.getContextVector(i).getCoveredText();
